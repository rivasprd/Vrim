(function( $ ) {
	...
	$.Shop.prototype = {
		init: function() {
			...
			this.cartPrefix = "Vrim-"; // Prefix string to be prepended to the cart's name in the session storage
			...
			this.paypalCurrency = "ARS"; // PayPal's currency code
		...
		populatePayPalForm: function() {
			...
			if( self.$paypalForm.length ) {
				...
				for( var i = 0; i < cartItems.length; ++i ) {
                    ...
                    insertBefore( "#paypal-btn" );
                    $( "<div/> ").html( "<input type='hidden' name='Color_'" + n + "' value='" + Color + "'/>").
					insertBefore( "#paypal-btn" );
			...
			if( this.$formCart.length ) {
				...
				if( items.length == 0 ) {
					...
					for( var i = 0; i < items.length; ++i ) {
						...
						var html = "<tr><td class='pname'>" + product + "</td>" + "<td class='pColor'>" + Color + "</td>" + "<td class='pqty'><input type='text' value='" + qty + "' class='qty'/></td>";
				...
				if( cartItems.length > 0 ) {
					...
					for( var j = 0; j < cartItems.length; ++j ) {
                        ...
                        var cartColor = cartItem.Color;
						var cartHTML = "<tr><td class='pname'>" + cartProduct + "</td>" + "</td>" + "<td class='pColor'>" + cartColor + "<td class='pqty'>" + cartQty + "</td>" + "<td class='pprice'>" + cartPrice + "</td> + </tr>";
		  ...
		  if( self.$updateCartBtn.length ) {
			self.$updateCartBtn.on( "click", function() {
				...
				$rows.each(function() {
                    ...
                    var pColor = $row.find( (".pColor").text() );
				...
				self.storage.setItem( self.shippingRates, self._convertNumber( self._calculateShipping( totalQty ) ) );
				self.storage.setItem( self.cartName, self._toJSONString( updatedCart ) );
                self.storage.setItem( self.cartColor )
